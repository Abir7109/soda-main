...
import { ToolStatus, RunCodeData } from '../types';

interface RunCodeProps {
  status: ToolStatus;
  data?: Partial<RunCodeData>;
}

const defaultData: RunCodeData = {
  code: `def calc_tactical(v):\n    return v * 1.414\n\nprint(calc_tactical(100))`,
  language: 'python',
  output: '141.4',
  success: true,
};

export default function RunCode({ status = 'done', data }: RunCodeProps) {
  const [jsonText, setJsonText] = React.useState(() => JSON.stringify(data || {
    code: '',
    language: 'python',
    output: 'STDOUT AWAITING TACTICAL SOURCE CODE SEQUENCE...',
    success: true,
  }, null, 2));
  ...
  const scanningLineStyle: React.CSSProperties = {
    ...
    animation: 'sodaSweep 3.4s linear infinite',
    pointerEvents: 'none',
    zIndex: 10,
  };
      ...
      <div style={cardStyle}>
        ...
        <div style={animZoneStyle}>
          ...
          <div style={{
            ...
            {isError ? 'ERR_CODE' : isRunning ? 'EVALLING' : 'SUCCESS'}
          </div>
          ...
          {isError ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', color: '#ffb4ab' }}>
              ...
              <span style={{ fontSize: '10px', fontWeight: 'bold', letterSpacing: '0.1em' }}>SYNTAX_ERROR</span>
            </div>
          ) : (
            <div style={{
              display: 'flex',
              ...
              alignItems: 'center',
              gap: '6px',
              animation: isRunning ? 'codePulse 1.6s infinite ease-in-out' : 'none'
            }}>
              <div style={{ fontSize: '48px', fontFamily: "'Space Grotesk', sans-serif", fontWeight: 'bold', color: '#00fbfb' }}>
                {'{  }'}
              </div>
              <span style={{
                fontFamily: "'Space Grotesk', sans-serif",
                ...
                {isRunning ? 'EXEC COMP_REFR' : 'COMPILER STANDBY'}
              </span>
            </div>
          )}
        </div>
        ...
        <div style={dataZoneStyle}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <div style={{
              ...
              padding: '4px',
              borderLeft: `1px solid ${strokeColor}44`,
              whiteSpace: 'pre-wrap',
            }}>
              {mergedData.code}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', marginTop: '4px' }}>
              <span style={labelStyle(0.3)}>COMPILED OUT /* STDOUT */</span>
              <div style={{
                ...
                {isError ? 'Traceback: IndentationError: unexpected indent' : isRunning ? 'Evaluating...' : mergedData.output}
              </div>
            </div>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: `1px dashed ${strokeColor}22`, paddingTop: '4px' }}>
            <span style={labelStyle()}>COMP_SUCCESS</span>
            <span style={{ ...valueStyle, color: isError ? '#ffb4ab' : successColor }}>
              {isError ? 'FALSE' : isRunning ? '--' : 'TRUE'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', width: '320px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <label style={{
            ...
            REAL DATA PIPELINE INPUT [run_code]
          </label>
          <span style={{
            fontSize: '8px',
